...
def get_hdd(id_registro: str):
    conexion = conexionDB()
    resultado = conexion.execute(hdds.select().where(hdds.c.idRegistro == id_registro)).first()
    conexion.close()
    if resultado:
        return resultado
...
@hdd.delete('/hdd/{id_registro}', status_code=HTTP_204_NO_CONTENT, tags=['HDD'])
def delete_hdd(id_registro: str):
    conexion = conexionDB()
    resultado = conexion.execute(hdds.delete().where(hdds.c.idRegistro == id_registro))
    conexion.close()
    if resultado:
