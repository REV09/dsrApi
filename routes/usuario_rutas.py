...
@usuario.get('/usuario/{nombre_usuario}', response_model=Usuario, tags=["Usuario"])
def get_usuario(nombre_usuario: str):
    llave_seguridad = cargar_llave()
    conexion = conexionDB()
    resultado = conexion.execute(usuarios.select().where(usuarios.c.nombreUsuario == nombre_usuario)).first()
    conexion.close()
    if resultado:
        usuario_obtenido = dict(resultado)
        usuario_obtenido["contrasena"] = desencriptar_mensaje(usuario_obtenido['contrasena'], llave_seguridad)
...
def add_usuario(usuario: Usuario):
    ...
    nuevo_usuario["contrasena"] = encriptar_mensaje(usuario.contrasena, llave_seguridad)
    conexion = conexionDB()
...
@usuario.delete('/usuario/{nombre_usuario}', status_code=HTTP_204_NO_CONTENT, tags=["Usuario"])
def delete_usuario(nombre_usuario: str):
    conexion = conexionDB()
    resultado = conexion.execute(usuarios.delete().where(usuarios.c.nombreUsuario == nombre_usuario))
    conexion.close()
    if resultado:
...
@usuario.put('/usuario/{nombre_usuario}', response_model=Usuario, tags=['Usuario'])
def update_usuario(nombre_usuario_anterior: str, nuevo_usuario: Usuario):
    llave_seguridad = cargar_llave()
    nueva_contrasena = encriptar_mensaje(nuevo_usuario.contrasena, llave_seguridad)
    conexion = conexionDB()
